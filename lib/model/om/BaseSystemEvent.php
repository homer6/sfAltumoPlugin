...
/**
 * Base class that represents a row from the 'system_event' table.
 *
 * 
 *
...
abstract class BaseSystemEvent extends BaseObject  implements Persistent
{
	...
	/**
	 * Initializes internal state of BaseSystemEvent object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     SystemEvent The current object (for fluent API support)
	 */
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SystemEventPeer::NAME;
		}
	...
	public function setUniqueKey($v)
	{
		...
		if ($this->unique_key !== $v) {
			$this->unique_key = $v;
			$this->modifiedColumns[] = SystemEventPeer::UNIQUE_KEY;
		}
	...
	public function setSlug($v)
	{
		...
		if ($this->slug !== $v) {
			$this->slug = $v;
			$this->modifiedColumns[] = SystemEventPeer::SLUG;
		}
	...
	public function setEnabled($v)
	{
		...
		if ($this->enabled !== $v || $this->isNew()) {
			$this->enabled = $v;
			$this->modifiedColumns[] = SystemEventPeer::ENABLED;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->unique_key = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			...
			return $startcol + 7; // 7 = SystemEventPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SystemEventPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($isInsert) {
				...
				// symfony_timestampable behavior
				if (!$this->isColumnModified(SystemEventPeer::CREATED_AT))
				{
			...
			if ($ret) {
				...
				SystemEventPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SystemEventPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['SystemEvent'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['SystemEvent'][$this->getPrimaryKey()] = true;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SystemEventPeer::NAME)) $criteria->add(SystemEventPeer::NAME, $this->name);
		if ($this->isColumnModified(SystemEventPeer::UNIQUE_KEY)) $criteria->add(SystemEventPeer::UNIQUE_KEY, $this->unique_key);
		if ($this->isColumnModified(SystemEventPeer::SLUG)) $criteria->add(SystemEventPeer::SLUG, $this->slug);
		if ($this->isColumnModified(SystemEventPeer::ENABLED)) $criteria->add(SystemEventPeer::ENABLED, $this->enabled);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of SystemEvent (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     SystemEvent Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this SystemEvent is new, it will return
	...
	{
		return (string) $this->exportTo(SystemEventPeer::DEFAULT_STRING_FORMAT);
	}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseSystemEvent:' . $name))
		{
