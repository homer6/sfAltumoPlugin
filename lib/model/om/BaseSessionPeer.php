...
abstract class BaseSessionPeer {
	...
	const CLASS_DEFAULT = 'plugins.sfAltumoPlugin.lib.model.Session';
	...
	/** the column name for the CLIENT_IP_ADDRESS field */
	const CLIENT_IP_ADDRESS = 'session.CLIENT_IP_ADDRESS';
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn(SessionPeer::SESSION_KEY);
			$criteria->addSelectColumn(SessionPeer::DATA);
	...
	{
		// Invalidate objects in SingleSignOnKeyPeer instance pool, 
	...
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += SessionPeer::doOnDeleteCascade(new Criteria(SessionPeer::DATABASE_NAME), $con);
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		try {
			...
			$c = clone $criteria;
			$affectedRows += SessionPeer::doOnDeleteCascade($c, $con);
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		$objects = SessionPeer::doSelect($criteria, $con);
		foreach ($objects as $obj) {
			...
			$criteria->add(SingleSignOnKeyPeer::SESSION_ID, $obj->getId());
	...
	{
	  return array(array('session_key'));
	}
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseSessionPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
