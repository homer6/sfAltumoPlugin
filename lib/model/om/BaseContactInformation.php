...
abstract class BaseContactInformation extends BaseObject  implements Persistent
{
  ...
  const PEER = 'ContactInformationPeer';
	...
	/**
	 * @var        array ProductSelection[] Collection to store aggregation of ProductSelection objects.
	 */
	protected $collProductSelections;
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 12; // 12 = ContactInformationPeer::NUM_COLUMNS - ContactInformationPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collProductSelections = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collProductSelections !== null) {
				foreach ($this->collProductSelections as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->aState) {
				$result['State'] = $this->aState->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setPhoneMainNumber($this->phone_main_number);
		$copyObj->setPhoneOtherNumber($this->phone_other_number);
		$copyObj->setMailingAddress($this->mailing_address);
		...
		if ($deepCopy) {
			...
			foreach ($this->getProductSelections() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addProductSelection($relObj->copy($deepCopy));
				}
			}
	 ...
	 * @return     void
	 */
	public function initClientsRelatedByDefaultBillingContactInformationId()
	{
	 ...
	 * @return     void
	 */
	public function initClientsRelatedByDefaultShippingContactInformationId()
	{
	...
	/**
	 * Clears out the collProductSelections collection
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addProductSelections()
	 */
	public function clearProductSelections()
	{
		$this->collProductSelections = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collProductSelections collection.
	 *
	 * By default this just sets the collProductSelections collection to an empty array (like clearcollProductSelections());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initProductSelections()
	{
		$this->collProductSelections = new PropelObjectCollection();
		$this->collProductSelections->setModel('ProductSelection');
	}

	/**
	 * Gets an array of ProductSelection objects which contain a foreign key that references this object.
	 *
	 * If the $criteria is not null, it is used to always fetch the results from the database.
	 * Otherwise the results are fetched from the database the first time, then cached.
	 * Next time the same method is called without $criteria, the cached collection is returned.
	 ...
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 * @return     PropelCollection|array ProductSelection[] List of ProductSelection objects
	 * @throws     PropelException
	 */
	public function getProductSelections($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collProductSelections || null !== $criteria) {
			if ($this->isNew() && null === $this->collProductSelections) {
				// return empty collection
				$this->initProductSelections();
			} else {
				$collProductSelections = ProductSelectionQuery::create(null, $criteria)
					->filterByContactInformation($this)
					->find($con);
				if (null !== $criteria) {
					return $collProductSelections;
				}
				$this->collProductSelections = $collProductSelections;
			}
		}
		return $this->collProductSelections;
	}

	/**
	 * Returns the number of related ProductSelection objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related ProductSelection objects.
	 * @throws     PropelException
	 */
	public function countProductSelections(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collProductSelections || null !== $criteria) {
			if ($this->isNew() && null === $this->collProductSelections) {
				return 0;
			} else {
				$query = ProductSelectionQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collProductSelections);
		}
	}

	/**
	 * Method called to associate a ProductSelection object to this object
	 * through the ProductSelection foreign key attribute.
	 *
	 * @param      ProductSelection $l ProductSelection
	 * @return     void
	 * @throws     PropelException
	 */
	public function addProductSelection(ProductSelection $l)
	{
		if ($this->collProductSelections === null) {
			$this->initProductSelections();
		}
		if (!$this->collProductSelections->contains($l)) { // only add it if the **same** object is not already associated
			$this->collProductSelections[]= $l;
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collClientsRelatedByDefaultBillingContactInformationId) {
				foreach ((array) $this->collClientsRelatedByDefaultBillingContactInformationId as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collClientsRelatedByDefaultShippingContactInformationId) {
				foreach ((array) $this->collClientsRelatedByDefaultShippingContactInformationId as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collProductSelections) {
				foreach ((array) $this->collProductSelections as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collUsers) {
