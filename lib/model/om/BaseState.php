...
abstract class BaseState extends BaseObject  implements Persistent
{
  ...
  const PEER = 'StatePeer';
	...
	/**
	 * The value for the iso_short_code field.
	 * @var        string
	 */
	protected $iso_short_code;
	...
	/**
	 * Get the [iso_short_code] column value.
	 * 
	 * @return     string
	 */
	public function getIsoShortCode()
	{
		return $this->iso_short_code;
	}
	...
	public function setIsoCode($v)
	{
		...
		if ($this->iso_code !== $v) {
			$this->iso_code = $v;
			$this->modifiedColumns[] = StatePeer::ISO_CODE;
		}
	...
	/**
	 * Set the value of [iso_short_code] column.
	 * 
	 * @param      string $v new value
	 * @return     State The current object (for fluent API support)
	 */
	public function setIsoShortCode($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->iso_short_code !== $v) {
			$this->iso_short_code = $v;
			$this->modifiedColumns[] = StatePeer::ISO_SHORT_CODE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->iso_code = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->iso_short_code = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			...
			return $startcol + 7; // 7 = StatePeer::NUM_COLUMNS - StatePeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(StatePeer::ID) ) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->aCountry) {
				$result['Country'] = $this->aCountry->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setIsoShortCode($arr[$keys[3]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(StatePeer::ISO_CODE)) $criteria->add(StatePeer::ISO_CODE, $this->iso_code);
		if ($this->isColumnModified(StatePeer::ISO_SHORT_CODE)) $criteria->add(StatePeer::ISO_SHORT_CODE, $this->iso_short_code);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setIsoShortCode($this->iso_short_code);
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collContactInformations) {
				foreach ((array) $this->collContactInformations as $o) {
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseState:' . $name))
		{
