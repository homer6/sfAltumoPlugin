...
abstract class BaseCountry extends BaseObject  implements Persistent
{
	...
	/**
	 * @var        array State[] Collection to store aggregation of State objects.
	 */
	...
	public function setIsoCode($v)
	{
		...
		if ($this->iso_code !== $v) {
			$this->iso_code = $v;
			$this->modifiedColumns[] = CountryPeer::ISO_CODE;
		}
	...
	public function setIsoShortCode($v)
	{
		...
		if ($this->iso_short_code !== $v) {
			...
			$this->modifiedColumns[] = CountryPeer::ISO_SHORT_CODE;
		}
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(CountryPeer::ID) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CountryPeer::ISO_CODE)) $criteria->add(CountryPeer::ISO_CODE, $this->iso_code);
		if ($this->isColumnModified(CountryPeer::ISO_SHORT_CODE)) $criteria->add(CountryPeer::ISO_SHORT_CODE, $this->iso_short_code);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setIsoShortCode($this->getIsoShortCode());
	 ...
	 * @return     PropelCollection|array State[] List of State objects
	 * @throws     PropelException
	 */
	...
	public function addState(State $l)
	{
		...
		if (!$this->collStates->contains($l)) { // only add it if the **same** object is not already associated
			$this->collStates[]= $l;
			$l->setCountry($this);
		}
	}
